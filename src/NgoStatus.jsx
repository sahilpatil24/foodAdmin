...
function NgoStatus() {
  ...
  useEffect(() => {
    // If no ID is found in the URL, set an error and stop
    if (!id) {
    ...
    // Create a reference to the specific document using the ID
    const ngoDocRef = doc(db, "NGOs", id);

    // Set up a real-time listener for that document
    const unsubscribeSnapshot = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          // Document does not exist, likely an invalid ID
          setStatus("Not Found");
  ...
  }, [id]); // The effect re-runs if the URL ID changes
    ...
    <div className="status-container">
      ...
      {status === "Approved" && (
        <div className="status-card approved">
          ...
          <p>
            Your NGO is now officially verified. You can now access all
            features.
          </p>
          <a href="/">Go to home</a>
